            ...
            <div class="page-header">
                <div class="page-header-left d-flex align-items-center">
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">Xodimlar ro'yhati</li>
                    </ul>
                </div>
                                            ...
                                            <tr>
                                                <td>
                                                    <div class="custom-control custom-checkbox">
                                                        <input type="checkbox" class="custom-control-input"
                                                               id="checkBox_{{ $tm->id }}">
                                                        <label class="custom-control-label"
                                                               for="checkBox_{{ $tm->id }}"></label>
                                                    </div>
                                                </td>
                                                <td>
                                                    <a href="{{ route('employees.show', $tm->id) }}"
                                                       class="hstack gap-3">
                                                        <div>
                                                            <span
                                                                class="text-truncate-1-line">{{ $tm->name }}</span>
                                                        </div>
                                                    </a>
                                                </td>
                                                <td>{{ $tm->branch->name ?? null}}</td>
                                                <td><a href="#">{{ $tm->pass }}</a></td>
                                                <td>
                                        
                                                    <select class="form-select form-control max-select"
                                                            data-select2-selector="tag" data-max-select2="tag" multiple>
                                                        @foreach ($tm->directions as $direction)
                                                            <option value="{{ $direction }}" data-bg="bg-success"
                                                                    selected>{{ $direction }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td><a href="tel:">{{ $tm->phone }}</a></td>
                                                <td>{{ $tm->created_at->format('Y-m-d') }}</td>
                                                <td>

                                                    <select class="form-control" data-select2-selector="status">
                                                        @if($tm->status=='Active')
                                                            <option value="success" data-bg="bg-success"
                                                                    selected>{{$tm->status}}</option>
                                                        @endif
                                                ...
                                                <td>
                                                    <div class="hstack gap-2 justify-content-end">
                                                        ...
                                                        <a href="{{ route('employees.edit', $tm->id) }}"
                                                           class="avatar-text avatar-md">
                                                        ...
                                                        <form id="deleteForm{{ $tm->id }}" action="{{ route('employees.destroy', $tm->id) }}" method="POST" onsubmit="return confirm('Ochirishga ruxsat berasizmi')">
                                                            @csrf
                                                            @method('DELETE')
